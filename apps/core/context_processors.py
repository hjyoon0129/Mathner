...
def profile_context(request):
    ...
    if request.user.is_authenticated:
        try:
            ...
            my_nickname = profile.nickname or request.user.username or getattr(request.user, "email", "") or "User"
            nav_star_count = int(profile.total_stars or 0)
            nav_key_count = profile.get_remaining_keys()
            needs_nickname = not bool(profile.nickname)

        except Exception:
            needs_nickname = False
            my_nickname = request.user.username or getattr(request.user, "email", "") or "User"
            nav_star_count = 0
            nav_key_count = 0
